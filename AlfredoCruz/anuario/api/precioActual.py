...
def apiPrecioActual(request,id=None):
	if request.method == 'GET':
		if id is not None:
			try:
				lista=[]
				query =precio_actual.objects.values('CurrencyISO3__nombre','DayEndNAVDate','DayEndNAV','MonthEndNAVDate','MonthEndNAV','UnsplitNAV','NAV52wkHigh','NAV52wkHighDate','NAV52wkLow','NAV52wkLowDate','PerformanceReturnSource').get(pk=id)
				lista.append({
